...
| 关键词 | 行为 |
|---|---|
| `fund` | 只有一个分组时直接查看该组；多个分组时列出分组供选择 |
...
- `fund`：**只有一个分组**时直接显示该组；**多个分组**时列出所有分组供选择，需输入序号后才查询（不发空查询）
- `fund 2`：切到**第 2 个**分组
- `fund N`：切到第 N 个分组（N 从 1 开始计数）
- 序号越界：多个分组时提示并回到分组选择列表；仅一个分组时提示后回落到该组继续显示
...
| 情况 | 行为 |
|---|---|
| `fund N` 序号越界 | 多个分组：提示越界并回到分组选择列表；仅一个分组：提示后回落到该组继续显示 |
